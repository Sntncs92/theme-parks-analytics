...
def generate_report(park_name: str, report_date, closing_time, output_dir: Path):
    """
    Llama al módulo de generación de reportes.
    Esta función es el punto de integración con charts.py y captions.py
    de la Semana 3. Por ahora genera un placeholder para validar el flujo.
    """
    # TODO (Semana 3): reemplazar este bloque por la llamada real a generate_report.py
    slug      = park_name.replace(" ", "_").replace("'", "").replace("é", "e")
    date_str  = report_date.isoformat()
    out_dir   = output_dir / date_str
    out_dir.mkdir(parents=True, exist_ok=True)

    placeholder = out_dir / f"{slug}_{date_str}.txt"
    placeholder.write_text(
        f"REPORTE PENDIENTE DE GENERACIÓN\n"
        f"Parque:  {park_name}\n"
        f"Fecha:   {date_str}\n"
        f"Cierre:  {closing_time}\n"
                ...
                for row in pending:
                    ...
                    try:
                        ...
                        out_path = generate_report(
                            park_name, report_date, closing_time, OUTPUT_DIR
                        )
